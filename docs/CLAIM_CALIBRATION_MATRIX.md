...
| Policy-environment interaction: 32-rollout MCTS underperforms Scout on standard maps yet outperforms Scout under bottleneck contention | README §CLI reference → evaluate; README §MCTS Empirical Evaluation (standard and contention-bearing); [`docs/articles/03-automated-map-fairness-profiling.md`](articles/03-automated-map-fairness-profiling.md) | `benchmarks/mcts_evaluation_results.json` (source revision `5783ca1`); `benchmarks/bottleneck_evaluation_results.json` (source revision `1c6fa80`); procedural generator + paired harness `c568897`, `cb91bc3`, `f4af28d`, artifact `e6ba4bf` | Mirrored-seat scoring per [`adr-003`](adr-003.md); 32 rollouts/action; standard: dev 50 + heldout 50 seeds (mean delta −1.12/−1.25, 0/100 seeds favorable to MCTS); bottleneck: dev 30 + heldout 30 seeds (mean delta +1.42/+1.38, 55/60 seeds favorable); macOS 8-core, .NET 10.0.10; ≤200 match steps | Empirical and context-dependent (see `FINDING-006`); hardware- and map-suite-scoped; implies no production-confidence claim and no extrapolation to longer rollouts or unlisted mapsets | `RESEARCH_PREVIEW_SCOPED` |
...
| Release immutability: `v2.3.0` and `v2.3.1` artifacts are pinned, checksummed, and reproducible from a single committed revision | README §Development and Verification Method (release verification); [`reproduction_packet.md`](reproduction_packet.md) (experiment 1 and appendices); [`SUPPORT_AND_REPRODUCIBILITY.md`](SUPPORT_AND_REPRODUCIBILITY.md) §5 | `.github/workflows/release.yml` (tag↔project parity, native smoke matrix, SHA-256 generation, SBOM emission); checked-in checksums and source commit pin in `docs/reproduction_packet.md`; tag `v2.3.1` → commit `c0e8342` | Release job runs the smoke matrix on ubuntu, windows, macos; checksums exercised by the reproduction packet verification procedures | Immutability covers the binary assets and their pinned source commit; it does not freeze `main` — post-release commits continue with `RESEARCH_PREVIEW_SCOPED` wording. Staging/publish defect history in `FINDING-003` | `CONFIRMED_BOUNDED` |
